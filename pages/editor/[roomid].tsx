...
const EditorContainer: React.FC<EditorProps> = ({}) => {
  ...
  function joinEventhandler({ clients, username, socketId }: any) {
    setClients(clients);
    console.log("From joinEventhandler", clients, username, socketId);
    if (username !== name) {
  ...
  useEffect(() => {
    console.log("From editor", name, roomId);
    if (!name || name === "") {
      router.push("/");
    }
  ...
  return (
    <div className="bg-bgdark px-2 flex text-white">
      <Head>
        <title>Editor | Code Collab</title>
      </Head>
      <div className="flex-1 grid grid- grid-cols-editor ">
        <div className="flex flex-col h-screen justify-between">
            ...
            <div className="flex-col  my-4 w-full ">
              {Object.keys(dummyFilesData).map((keyName, i) => {
                ...
                return (
                  ...
                  >
                    <Image width={20} height={20} src={fileData.iconName} />
          ...
          <div className="mx-3">
            ...
            <button
              onClick={leaveRoom}
              className="w-full rounded-xl p-3 mb-2 font-bold bg-yellow-500 text-black"
            >
              Leave
            </button>
          </div>
